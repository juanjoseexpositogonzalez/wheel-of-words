...
| REQ-002-006 | Agregación por forma normalizada con frecuencia y orden (§2.4) | `openspec/changes/text-import/specs/002-text-import/spec.md` — AC-002-08, AC-002-09 | `apps/api/tests/unit/test_frequency.py::test_repeated_forms_collapse_with_frequency_and_sum`, `apps/api/tests/api/test_imports.py::test_rows_arrive_already_ordered_by_the_grouping_key` | T1A07–T1A09 (leg de dominio); T1B13–T1B16 (leg de respuesta `POST`); T211 pendiente | En progreso — los legs de dominio (corte 1a) y de respuesta `POST` (corte 1b) están cubiertos; se completa en el corte 2 con la ruta de lectura `GET` (spec §1.2, requisito transversal) |
| REQ-002-018 | Cada grupo muestra una forma de superficie real, elegida de forma determinista (D1–D3) | `openspec/changes/text-import/specs/002-text-import/spec.md` — AC-002-23, AC-002-24 | `apps/api/tests/unit/test_frequency.py::test_majority_and_tie_break_display_form`, `apps/api/tests/api/test_imports.py::test_each_row_carries_both_the_grouping_key_and_the_display_form` | T1A07–T1A09 (leg de dominio); T1B15–T1B16 (`display_form` en la respuesta `POST`); T202 pendiente | En progreso — los legs de dominio (corte 1a) y de respuesta `POST` (corte 1b) están cubiertos; se completa en el corte 2 con la ruta de lectura `GET` (spec §1.2, requisito transversal) |
...
| REQ-002-007 | Ni la clave de agrupación ni la forma mostrada se llaman «lemma» ni «lexeme» | `openspec/changes/text-import/specs/002-text-import/spec.md` — AC-002-10 | `apps/api/tests/unit/test_no_lemma_naming.py` (leg de backend); `apps/web/tests/contracts/no-lemma-naming.test.ts` (leg de frontend, incluye el describe `findViolations (remediation — pins the comment exemption directly)`) | T1B20 (leg de backend); T1C14 (leg de frontend); T203 pendiente | En progreso — los legs de backend (corte 1b) y de frontend (corte 1c) están cubiertos; el leg de columnas persistidas sigue pendiente y se completa en el corte 2 (spec §1.2, requisito transversal). Ambos legs shipeados son ahora estructurales (AST): el de backend usa el módulo `ast` de Python (identificadores + literales no-docstring) más claves JSON y el documento OpenAPI servido; el de frontend usa la API del compilador de `typescript` (ya dependencia de `apps/web`) para el mismo criterio sobre identificadores y literales de cadena no-comentario, incluyendo texto JSX y literales de plantilla. Corregido por remediación (véase `tasks.md`, nota 5): el leg de frontend originalmente shipeó como búsqueda de texto plano sin exención de comentarios, lo cual reintroducía la regresión que motivó convertir el leg de backend de grep a AST en el corte 1b; ambos legs están ahora unificados en el mismo criterio AST. Segunda remediación (véase `tasks.md`, nota 6): la exención de comentarios del leg de frontend — antes probada una sola vez por mutación manual y documentada solo en un comentario de código — quedó fijada por 5 pruebas de regresión permanentes sobre `findViolations` (exportada sin cambio de lógica), cubriendo ambas direcciones: comentario `//`/bloque → cero violaciones; identificador, literal de cadena, literal de plantilla y texto JSX → exactamente una violación cada uno, con `kind`, texto y línea verificados |
| REQ-002-013 | Ningún registro de log contiene texto importado; los fallos se identifican por código e `import_id` | `openspec/changes/text-import/specs/002-text-import/spec.md` — AC-002-18 | `apps/api/tests/api/test_imports_logging.py` | T1B18–T1B19 (leg de importación); T212 pendiente | En progreso — solo el leg de la ruta de importación (corte 1b) está cubierto; se completa en el corte 2 con los logs de lectura y persistencia (spec §1.2, requisito transversal) |
