...
Acceptance: **AC-005-08** — Given every module this capability introduces, when the structural
write-detector scans them, then the scan reaches every module in scope (no blanket exemption of this
capability's own modules from the walk, per §5 AMB-3); it reports a violation for every `INSERT`,
`UPDATE` or `DELETE` targeting `manual_correction` the detector is built to detect — through the ORM
class or as literal SQL text — except the single exemption its own test module documents, whose
boundary an exact-set-equality test enforces, that test itself now protected from deletion the same
way every other pinning test is (round 15); and every test that pins a mechanism or a bound this
detector implements is kept true — computed at test time from two directions, not a hand-written
list (round 13, re-derived round 15): every `test_*` name §5 AMB-15's row cites by exact name must
still exist in the test module, and every test the module itself tags MUTATION CHECK must be cited
in that row, so a cited test cannot be deleted and a new pinning test cannot go unregistered without
either direction failing, by name; neither direction can detect a citation and the test it names
being removed together in the same edit, and this document does not claim it can — that removal is
visible only in a reviewed diff spanning this file and the test module; the test module is the
authoritative record of the detector's coverage and of its bounds — bounds recorded there are known
and explicitly not claimed exhaustive (§5 AMB-15 collects the bounds found through round 12:
dynamically-assembled SQL text, the one-hop binding tracker's and write-verb alias map's syntactic
limits, and the raw-SQL regex's tolerance in both directions, among others — recorded as examples
the test module pins, not a list this acceptance depends on being complete); and given the
vocabulary repository with a seeded correction present, when the vocabulary read runs, then the
`manual_correction` rows are byte-identical afterwards and the row count is unchanged; and given
each of two mutations applied in turn — an insert against `ManualCorrection` and a delete against
it, both inside the vocabulary repository — when the guard runs, then **each** produces a violation,
with the observed failure output recorded in the test docstring; and given the same statement placed
in a module outside this capability, then it still produces a violation (M3); and given the frontend
client module this capability introduces, when its requests are enumerated, then every one is a
`GET`; and given the rendered vocabulary view, when its interactive controls are enumerated, then
none submits a correction.

#### Scenario: The structural write guard runs over every module in scope, catches what it is built to catch, and keeps its coverage and its exemptions provably enforced

- GIVEN every module this capability introduces
- WHEN the structural write-detector scans them
- THEN the scan reaches every module in scope — a module dropped from the scan is a failure of this
  scenario, not a silent gap (§5 AMB-3)
- AND it reports a violation for every `INSERT`, `UPDATE` or `DELETE` targeting `manual_correction`
  the detector is built to detect, through the ORM class or as literal SQL text, except the single
  exemption its own test module documents — an unexempted violation reaching production is a
  failure of this scenario
- AND every test that pins a mechanism or a bound this detector implements is kept true by two
  checks computed at test time, not a hand-written list (round 13, re-derived round 15):
  `test_every_named_pinning_test_still_exists_in_this_module` requires every `test_*` name §5
  AMB-15's row cites by exact name to still exist in the test module — deleting a cited test is a
  failure of this scenario, verified by execution; `test_every_mutation_check_test_is_cited_in_amb15`
  requires every test the module itself tags MUTATION CHECK to be cited in that row — adding a new
  pinning test without registering its citation is a failure of this scenario, verified by
  execution. Together the two close the gap round 12 left open more completely than round 13's
  hand-maintained list did, because neither direction depends on a human remembering to update a
  constant; neither can, and structurally cannot, detect a citation being removed FROM §5 AMB-15's
  row together with the test it names being deleted in the same edit, because any test enforcing
  that would itself be an equally editable document one level up; that removal is visible only in a
  reviewed diff spanning this file and the test module, not caught by any test, and this scenario
  does not claim otherwise; a mechanism or a bound neither direction yet reaches is a mechanism or
  bound not yet anchored, not one this scenario claims is covered
- AND the exempt set changes only together with the test enforcing its exact membership — an
  exemption added without that boundary control is a failure of this scenario; that enforcing test,
  `test_the_exempt_set_is_exactly_book_repository`, is itself now cited in §5 AMB-15's row (round
  15), so deleting it before adding the exemption is caught by
  `test_every_named_pinning_test_still_exists_in_this_module` above, closing the combination round
  15 found passing green: the enforcer deleted and an exemption added in the same change
...
| **AMB-15** | **Eleven Judgment Day rounds enumerated AC-005-08 scenario 1 as an absolute THEN clause plus an exception list (`AMB-11` through `AMB-14`, registered across rounds 8 and 10), and each round after the first found a bound the enumeration did not carry.** Round 11 found a fifth class the four rows did not name — five over-approximations, each verified by execution: any callable literally named a write verb is flagged regardless of receiver or origin (`test_no_receiver_or_origin_is_verified_a_known_over_approximation`); an unrelated import renamed to a write-verb name is flagged (`test_renaming_an_unrelated_import_to_a_write_verb_name_is_an_over_approximation`); a class literally named `ManualCorrection` from any module is flagged (`test_an_unrelated_class_genuinely_named_manual_correction_is_flagged`); a different class aliased to that exact spelling is flagged (`test_an_import_aliasing_a_different_class_to_the_exact_name_is_flagged`); class-alias resolution is not flow-sensitive (`test_a_rebound_class_alias_is_still_treated_as_manual_correction`) — plus a folded-chain deduplication guarantee, not a bound (`test_a_folded_chain_is_reported_once_not_once_per_sub_expression`). Round 11 also found a false premise inside `AMB-14` itself: its under-match reason, 'SQLite has no schema support', is false — `main` and `temp` are built-in SQLite schemas needing no `ATTACH`, `DELETE FROM "main"."manual_correction"` and the backtick-quoted form both execute against plain `sqlite3` (verified by execution), `database_url` is an env-overridable pydantic setting (`infrastructure/settings.py`), and `create_engine_from_url` imposes no dialect restriction (`infrastructure/persistence/engine.py`); the same test module's own `test_the_raw_sql_adjacency_gaps_named_in_the_brief_are_now_closed` already parametrizes `text("DELETE FROM main.manual_correction")` under the id `"schema-qualified"` and asserts it IS caught, contradicting the withdrawn premise. An enumeration of a static analyser's bounds is structurally incomplete — every additional pass finds a bound the list does not carry, which is a specification violation by the enumeration's shape, not by any round's carelessness. | **The criterion's shape is changed, not its substance (round 12).** AC-005-08 scenario 1 no longer states an absolute THEN clause plus an enumerated exception list; it states what the check DOES: the structural detector runs over every module in scope and reports no unexempted violation, every mechanism and every bound the detector uses is defined and kept true by a NAMED test in its own test module, and that module — not this row, not the scenario text — is the authoritative record of the detector's coverage and of its bounds. The detector's bounds ARE known, recorded and pinned, and are explicitly NOT claimed exhaustive: a bound this round has not found is a bound not yet found, not a bound this scenario asserts does not exist. What follows is the recorded set of bounds as of round 12, kept as EXAMPLES the test module pins, not a list the acceptance criterion depends on being complete: dynamic SQL-text assembly (`%`-format, `str.join`, an interpolated f-string) is not resolved by static inspection (`test_dynamic_sql_assembly_is_a_documented_gap`); the one-hop binding tracker never registers an attribute/subscript target, never follows a second hop, and leaks across lexical scopes as a module-wide set (`test_a_non_name_assignment_target_is_a_documented_uncovered_boundary`, `test_a_second_assignment_hop_is_the_documented_uncovered_boundary`, `test_tuple_unpacking_targets_are_a_documented_uncovered_boundary`, `test_a_for_loop_target_is_a_documented_uncovered_boundary`, `test_the_one_hop_tracker_leaks_across_lexical_scopes_a_known_over_approximation`); a write verb reached through a REBOUND, not renamed, local name is not resolved (`test_a_rebound_write_verb_is_a_documented_uncovered_boundary`); the raw-SQL regex over-matches prose carrying a write-verb keyword with no contiguous fragment and under-matches a table reference with the schema and the table independently quoted (`test_prose_matching_the_tolerant_raw_sql_regex_is_flagged_a_known_over_approximation`, `test_a_fully_quoted_schema_and_table_name_is_a_documented_gap` — the latter's reachability inside this repository is corrected per the false-premise finding above: it is NOT provably unreachable, `main`/`temp` are built-in schemas and the under-match form executes against this project's own database engine unmodified); the five over-approximations and the deduplication guarantee round 11 found are pinned by the tests named in the GAP column. Every detection mechanism the detector implements and every tolerance component of its raw-SQL patterns is now independently mutation-pinned in `test_vocabulary_write_guard.py` — round 12 closed the eighteen tolerance components that had been exercised only in combination, the `_folded_string` fold arm, and the `_names_manual_correction` attribute arm (round 12 evidence, this Judgment Day report) — and the module's own non-vacuity anchors are hardcoded independently of the constants they check, the fix already applied to `_WRITE_VERBS` and `_FORBIDDEN_RAW_SQL`, so a mechanism silently dropping out of the detector removes a test's assertion target, not the test's own case. | **Registered per `AGENTS.md` §9 (round 12). Replaces `AMB-11` through `AMB-14`. AC-005-08 scenario 1 reshaped below to state the detector's behavior directly rather than enumerate its bounds.** **Round 13 found the reshaped scenario's own enforcement claim — "a mechanism or a bound whose enforcing test is missing or deleted is a failure of this scenario" — had no enforcing mechanism: deleting `test_a_module_qualified_manual_correction_reference_requires_the_attribute_arm` and applying the mutation it pins together left the 108-test module fully green (107 passed, zero red; control with the test present: 1 failed, 107 passed), a G1 violation inside the criterion round 12 reshaped to eliminate G1 violations. `_MECHANISM_AND_BOUND_PINNING_TESTS` and its guard test, `test_every_named_pinning_test_still_exists_in_this_module`, now enforce the "test deleted" half of that claim by execution for every test AMB-15 cites by name plus every MUTATION CHECK test not already cited there (27 names, verified individually this round: deleting each fails the guard by that name, and only that name). The "name removed from the anchor" half is not enforced and cannot be — any guard checking the anchor is itself an equally editable constant one level up. AC-005-08 scenario 1 is narrowed below to state exactly this bounded guarantee, not the unbounded one round 12 stated.** **Round 15 found the round-13 anchor was itself the same shape of gap it was built to close: a hand-written frozenset, already short by one the day it was written (`test_every_named_pinning_test_still_exists_in_this_module` is cited in this row's own text above but was never added to the anchor it names), and three further tests were found pinning a mechanism or a bound while cited nowhere and tagged nothing — `test_the_scan_reaches_every_expected_module` (the only test proving the real filesystem, not a synthetic tree, still contains what `_EXPECTED_PACKAGE_MODULES`/`_EXPECTED_MIGRATIONS` name), `test_the_scan_fails_closed_when_an_anchor_is_missing` (the package anchor's per-element fail-closed check), and `test_the_exempt_set_is_exactly_book_repository` (the exempt-set exact-membership boundary — this scenario's fourth failure mode's sole enforcer, so deleting it and adding an unauthorized exemption passed green, round 15 evidence, this Judgment Day report). A parallel gap was found in `_EXPECTED_MIGRATIONS` itself: unlike `_EXPECTED_PACKAGE_MODULES`, which has had a per-element fail-closed test since round 12, `_EXPECTED_MIGRATIONS` had none — `test_the_scan_reaches_every_expected_module`'s aggregate subtraction check is vacuously true when that constant is emptied (109/109 passed against the emptied constant, round 15 evidence), closed by the new `test_the_migrations_scan_fails_closed_when_an_anchor_is_missing`. The round-13 constant, `_MECHANISM_AND_BOUND_PINNING_TESTS`, is retired from the code. In its place, two checks are computed at test time, both reading from disk, neither from `globals()` alone: `test_every_named_pinning_test_still_exists_in_this_module` requires every `test_*` name this row cites by exact backtick name to exist in the test module; `test_every_mutation_check_test_is_cited_in_amb15` requires every test the module itself tags `MUTATION CHECK` in its own docstring to be cited somewhere in this row. Fifteen names are added to this row's citations by that second check's own enforcement (round 15): `test_a_docstring_containing_the_forbidden_fragment_is_flagged`, `test_a_fragment_split_across_a_plus_concatenation_requires_the_fold_arm`, `test_emptying_the_exempt_set_fails_through_write_violations`, `test_every_mutation_check_test_is_cited_in_amb15`, `test_every_raw_sql_fragment_is_caught`, `test_every_tolerance_component_is_independently_pinned_per_pattern`, `test_every_write_verb_is_caught_bare`, `test_every_write_verb_is_caught_under_a_renamed_import`, `test_the_exempt_set_is_exactly_book_repository`, `test_the_exemption_hides_a_genuine_write`, `test_the_migrations_scan_fails_closed_when_an_anchor_is_missing`, `test_the_scan_fails_closed_when_an_anchor_is_missing`, `test_the_scan_reaches_every_expected_module`, `test_the_two_quote_slots_in_the_raw_sql_regex_are_independently_pinned`, `test_writes_appended_to_the_vocabulary_repository_are_caught`. Combined, the two directions close AC-005-08 scenario 1's third failure mode more completely than round 13's hand list did — deleting ANY test that pins a mechanism or a bound now fails, not only the tests a human remembered to list — and close the fourth failure mode outright: its sole enforcer is cited here, so direction A now protects its own existence. What remains open is unchanged in kind from round 13, restated for the new mechanism: removing a citation from this row's text AND deleting the test it names, in the same edit, is not detectable by any test in this file, because the citation is prose in a document a test can read but not police against its own future edits — that removal is visible only in a reviewed diff of this row, exactly as round 13 already stated for the prior constant. Separately, round 15 verified by execution against plain `sqlite3` that the test module's own `test_a_fully_quoted_schema_and_table_name_is_a_documented_gap` carried the same withdrawn "SQLite has no schema support" premise this row's Round 11 paragraph above already corrected — `main`/`temp` need no `ATTACH DATABASE` and both quoted forms execute successfully; the test module's docstring is corrected to match this row, not the other way around, closing a drift this row's own text did not carry.** |
