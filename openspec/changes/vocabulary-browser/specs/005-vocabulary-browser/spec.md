...
boundary an exact-set-equality test enforces; and every test named in the test module's
`_MECHANISM_AND_BOUND_PINNING_TESTS` anchor — the union of every test §5 AMB-15 cites by name and
every test that module tags MUTATION CHECK — is kept true by that module's own guard test, which
fails, by name, if any anchored test is deleted (round 13); that guard cannot detect a name removed
FROM the anchor, as opposed to the test it names being deleted, and this document does not claim it
can — that removal is visible only in a reviewed diff of a named constant; the test module is the
...
- AND every test the detector's own test module names in `_MECHANISM_AND_BOUND_PINNING_TESTS` — the
  union of every test §5 AMB-15 cites by name and every test that module tags MUTATION CHECK — is
  kept true by that module's `test_every_named_pinning_test_still_exists_in_this_module` guard:
  deleting any one of those named tests is a failure of this scenario, verified by execution (round
  13); this closes HALF of the gap round 12 left open — it does not, and structurally cannot,
  detect a name being removed FROM `_MECHANISM_AND_BOUND_PINNING_TESTS` instead of the test it names
  being deleted, because any test enforcing that would itself be an equally editable constant one
  level up; that removal is visible only in a reviewed diff of a named constant, not caught by any
  test, and this scenario does not claim otherwise; a mechanism or a bound the anchor does not yet
  name is a mechanism or bound not yet anchored, not one this scenario claims is covered
- AND the exempt set changes only together with the test enforcing its exact membership — an
  exemption added without that boundary control is a failure of this scenario
