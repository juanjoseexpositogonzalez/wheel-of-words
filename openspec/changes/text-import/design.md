...
| Integration, bench | `T-BENCH`: generated synthetic corpus at the 4 MiB ceiling; asserts import wall time (§3.3), `GET` total (§3.4.5), response row count and body size (§3.4.3), and the **aggregation segment** against the 250 ms p95 trigger (§3.5) | `integration` | fixture generated in-test, never committed (Art. IV.1–2, H6). Lands in cut 2, the first cut with persistence |
