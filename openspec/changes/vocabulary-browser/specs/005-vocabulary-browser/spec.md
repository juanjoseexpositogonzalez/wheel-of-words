...
inspection, not a claim this acceptance makes); and given the vocabulary repository with a
...
- THEN none contains an `INSERT`, `UPDATE` or `DELETE` targeting `manual_correction`, expressed
  through the ORM class or as literal SQL text (§5 AMB-11)
...
| **AMB-11** | **AC-005-08 scenario 1 requires structural inspection to find "no `INSERT`, `UPDATE` or `DELETE` targeting `manual_correction`, whether expressed through the ORM class or as SQL text", with no qualifier on how that SQL text comes to exist in source.** Judgment Day round 8 closed the guard's raw-SQL adjacency gaps — whitespace, a newline, a quoted table name, a `main.`-qualified name, `REPLACE INTO`, `TRUNCATE TABLE` (`test_vocabulary_write_guard.py::test_the_raw_sql_adjacency_gaps_named_in_the_brief_are_now_closed`) — and verified, by executing every verb-times-channel combination the acceptance text names, that all six are detected (round 8 evidence, this Judgment Day report). What remains open is a DIFFERENT, narrower channel: a DELETE/INSERT/UPDATE statement whose text is assembled at runtime — `%`-format substitution, `str.join`, or an f-string with an interpolated `{...}` placeholder — never places the literal substring `manual_correction` contiguously in any AST string constant the scan visits, so it evades detection even though it is, at execution time, exactly the SQL text the scenario forbids (`test_dynamic_sql_assembly_is_a_documented_gap`, round 8). | **Not the same defect as the now-withdrawn original AMB-11.** That entry rested on the premise that an AST pass cannot verify a `Session` receiver; round 6 falsified it — receiver identity was never the blocker, and every ORM-instance idiom is now caught by matching the callee name plus `ManualCorrection` anywhere in the call expression, no receiver inference required. This entry is a different, genuine boundary: closing dynamically-assembled SQL text would require constant propagation or symbolic execution over arbitrary Python to resolve what a variable, a format substitution, or a join ultimately evaluates to at runtime — the same category of unverifiable inference the withdrawn original AMB-11 and three earlier Judgment Day rounds escalated on, this time for string VALUES rather than receiver TYPES. The requirement is not weakened to accommodate it: every `INSERT`/`UPDATE`/`DELETE` expressed as literal SQL text (including every adjacency variant round 8 closed) and every write expressed through the ORM class, regardless of receiver, is caught — confirmed by executing the full six-form matrix AC-005-08's text names. AC-005-08 scenario 1 is amended below to scope "as SQL text" to literal SQL text explicitly, so the acceptance criterion states the boundary the shipped guard actually enforces rather than a broader claim no static scan can make. | **Registered per `AGENTS.md` §9. AC-005-08 scenario 1 amended below to name the literal-text scope explicitly. REQ-005-008 is otherwise fully discharged: every ORM-class and literal-SQL-text form the acceptance text names is detected (round 8 evidence)** |
