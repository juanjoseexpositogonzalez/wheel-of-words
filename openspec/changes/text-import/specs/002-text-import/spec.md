...
Capability `002-text-import` is new: `openspec/specs/` contains no prior spec for it, so every
requirement below is ADDED. This delta expands `REQ-002-001` … `REQ-002-017` from
`openspec/changes/text-import/proposal.md` into verifiable requirements, acceptance criteria
(`AC-002-nn`), and scenarios.

**Amendment 1 (maintainer review).** `REQ-002-018` was added to resolve PV-1: the grouping key stays
the casefolded normalized form, but the row *displayed* to the user is a real surface form from the
text. `AMB-2` is settled as confirmation-only, permanent deletion. `AMB-3` is accepted.
`AMB-1`, `AMB-4`, `AMB-5` are accepted as originally specified. §2.3 `N1`–`N5` are unchanged.

**Amendment 2 (post-design, maintainer review).** The default size limit drops from 10 MiB to
**4 MiB** (`REQ-002-003`). `CONTRA-1`, raised by the design against Art. IX.6, is **resolved as not a
contradiction** (§5). Cut 1 was split and `REQ-002-003` was pulled out of the hardening cut. No
requirement text other than `REQ-002-003` changes meaning; §2.2 `T1`–`T10`, §2.3 `N1`–`N5`, and
§2.5 `D1`–`D3` are untouched. **The cut allocation Amendment 2 proposed is superseded by Amendment 3 — read §1.2,
not this note, for the delivery shape.**

**Amendment 3 (reconciliation with `design.md`).** Amendment 2 and the design amendment were run in
parallel over content the two artifacts share, and they diverged: this spec described four cuts
(`1a`, `1b`, `2`, `3`) while the design landed on five and dissolved the "hardening" cut entirely.
The design owns the sizing work and is downstream, so **the spec reconciles to the design**. §1.2 now
carries the authoritative five-cut allocation (`1a`, `1b`, `1c`, `2`, `3`), none of which needs a
`size:exception`. This amendment is reconciliation only: no requirement changes meaning, none is
added or removed, and `T1`–`T10`, `N1`–`N5`, `D1`–`D3` remain untouched.
...
| Requirement prefix | `REQ-002-###` (inherited verbatim from the proposal) |
| Acceptance prefix | `AC-002-##` |
| Governing constitution | v2.0.0 (2026-07-15, multi-language amendment 2026-07-16) |
| Repo state | `main` @ `aefbcf0` |
| Persistence mode | `openspec` + `engram` (`sdd/text-import/spec`) |
| Language | English (methodology artifact, ADR-0010). Product docs stay Spanish. |
| Delivery | **Five vertical cuts** (`1a`, `1b`, `1c`, `2`, `3`), `stacked-to-main`, **no `size:exception` required** — see §1.2 (Amendment 3, reconciled with `design.md`) |
...
Per `REQ-002-007` and Art. V.1, this slice groups by *normalized form*; **lemma** and **lexeme**
name a different concept and MUST NOT appear in any API field, response key, or UI string.

### 1.2 Requirement → acceptance → cut map

Authoritative allocation (Amendment 3, reconciled with `design.md`). Five cuts, none requiring a
`size:exception`, all inside the 400–700 authored-line band.

| REQ | Intent | AC | Cut |
|-----|--------|-----|-----|
| REQ-002-001 | Upload-only `.txt` intake | AC-002-01 | 1b |
| REQ-002-002 | Extension / content-type validation before processing | AC-002-02 | 1b |
| REQ-002-003 | Configurable size limit, reject oversized | AC-002-03, AC-002-04 | 1b |
| REQ-002-004 | Strict UTF-8 decode with actionable rejection | AC-002-05 | 1b |
| REQ-002-005 | Pure, language-generic tokenization + `normalize()` | AC-002-06, AC-002-07 | 1a |
| REQ-002-006 | Aggregate by normalized form, sorted, with frequency | AC-002-08, AC-002-09 | **1a + 1b + 2** |
| REQ-002-007 | Never label the grouping or the display form "lemma"/"lexeme" | AC-002-10 | **1b + 1c + 2** |
| REQ-002-008 | Persist `Book` + `Occurrence` via repository + migration | AC-002-11, AC-002-12 | 2 |
| REQ-002-009 | Cryptographic `content_hash` on `Book` | AC-002-13 | 2 |
| REQ-002-010 | Per-occurrence `pos` reserved, `None`, never global | AC-002-14 | 2 |
| REQ-002-011 | Delete import and derived data, confirmed | AC-002-15, AC-002-16 | 3 |
| REQ-002-012 | Empty file succeeds with zero unique forms | AC-002-17 | 1b |
| REQ-002-013 | Logs never contain raw imported text | AC-002-18 | **1b (partial) + 2 (closed)** |
| REQ-002-014 | Frontend duplicates no linguistic rules | AC-002-19 | 1c |
| REQ-002-015 | `normalize` is idempotent | AC-002-20 | 1a |
| REQ-002-016 | Unique-form set **and display forms** independent of input order | AC-002-21 | 1a |
| REQ-002-017 | Frequencies are never negative | AC-002-22 | 1a |
| REQ-002-018 | Display form is a real surface form, chosen deterministically | AC-002-23, AC-002-24 | **1a + 1b + 2** |

| Cut | Name | Kind | Requirements | Est. lines |
|-----|------|------|--------------|------------|
| **1a** | language engine | *verificable* | `-005`, `-015`, `-016`, `-017`, plus the **domain half** of `-006` and `-018` | ~435 |
| **1b** | callable import | *verificable* | `-001`, `-002`, `-003`, `-004`, `-012`, `-007` (backend), `-013` (partial), plus the **response half** of `-006` and `-018` | ~660 |
| **1c** | visible import | *observable* | `-014`, `-007` (frontend) | ~525 |
...
| REQ | 1a | 1b | 1c | Complete at 2 |
|-----|----|----|----|---------------|
| `-006` | domain aggregation | `POST` response shape (`AC-002-08`) | — | `GET` read path (`AC-002-09`) |
| `-007` | — | backend names and keys | frontend strings and labels | persisted column names |
| `-013` | — | partial: import-path logs | — | closed: read and persistence logs |
| `-018` | domain selection (`D1`–`D3`) | `display_form` in the `POST` response | — | `GET` read path (`AC-002-24`) |

Every other requirement is fully verifiable inside its single named cut.

**Why cuts 1a and 1b ship no UI, and why that is compliant.** `docs/constitution.md:41` reads
"Cada corte debe producir un resultado observable **o** verificable" — a disjunction. This is the
third disjunction this cycle, after Art. IX.5 (`AMB-2`) and Art. IX.6 (`CONTRA-1`). A cut proven by
its test suite is *verificable* and therefore compliant even with no screen to look at; `observable`
is one permitted way to satisfy the article, not the only one. Each cut is tagged `verificable` or
`observable` in the table above. Two consecutive UI-less cuts out of five is also not a "fase
horizontal larga sin valor integrado" under Art. III.3 (`docs/constitution.md:42`) — the
layer-by-layer plan the proposal rejected had four. **Do not re-litigate: Art. III.2 is a
disjunction.**

**Why the hardening cut dissolved.** `REQ-002-002`, `REQ-002-003`, and `REQ-002-004` no longer form
a separate hardening cut; all three land in `1b`, the first cut that ships a public upload endpoint
to `main` under `stacked-to-main`. An endpoint reachable from `main` must reject bad input properly
on the day it lands, not one cut later:

| REQ | Cost of deferring it past `1b` |
|-----|--------------------------------|
| `-002` file type | An unvalidated upload path in `main` for a whole cut |
| `-003` size limit | An upload endpoint with **no bound on request size** — an unbounded-memory path in `main`. A real configurable limit is cheap and removes the hazard properly, rather than patching it with a hard-coded defensive cap that a later cut has to find and unwind |
| `-004` encoding | Either a generic 422 for a Latin-1 file (violating Art. VIII.4, "Los errores de usuario son comprensibles") or a raw `UnicodeDecodeError` leaking a byte offset of the user's text into the response (violating Art. X.2) |

`REQ-002-009` moves to cut `2` because a content hash is only meaningful once a `Book` row exists to
carry it. `REQ-002-014` and the frontend side of `REQ-002-007` become cut `1c`, the first cut with a
UI.
...
synthetic `.txt` part, then the response is HTTP 201 with a frequency-table body (and, from cut 2
onward, an import `id`); and when a client posts a JSON body carrying a filesystem path instead of a
file part, then the response is HTTP 422 and nothing is imported — in cut 1b no frequency table is
returned, and from cut 2 onward no `Book` row is created.
...
is imported — in cut 1b no frequency table is returned, and from cut 2 onward no `Book` row and no
`Occurrence` row exist.
...
**Why 4 MiB, not the 10 MiB of Amendment 1 (maintainer product decision, post-design).** The design
quantified a 10 MiB import at roughly 1.72 M occurrence rows and about 12 s of synchronous request
time, which invites proxy and browser timeouts on a path that has no asynchronous state machine
(§5 CONTRA-1). 4 MiB is roughly 688 k tokens and about 3.4 s, and still clears *War and Peace*
(≈3.2 MB of plain text, public domain) with margin. The value remains one setting, so a user with a
larger corpus and a tolerant proxy can raise it.
...
— in cut 1b no frequency table is returned, and from cut 2 onward no `Book` row exists.
...
message contains `UTF-8` and conversion guidance, and nothing is imported (in cut 1b no frequency
table is returned; from cut 2 onward no `Book` row exists); and when a client
uploads UTF-8 bytes prefixed with `EF BB BF`, then the response is HTTP 201 and the first normalized
form does not begin with `U+FEFF`.
...
All are **closed** at maintainer review; the Status column records the outcome.
...
| AMB-3 | No maximum file size exists anywhere in the docs (explore OQ-4 unanswered). | Default `4194304` bytes (4 MiB) — above *War and Peace* (≈3.2 MB plain text, public domain), below the synchronous-timeout risk the design quantified. | Accepted, **revised by Amendment 2** from 10 MiB. Configurable via one setting |
...
| PV-7 | Default size limit **4 MiB** (AMB-3, revised by Amendment 2) | Uploads above 4 MiB are refused with the limit named. Roughly 688 k tokens and ≈3.4 s of synchronous work; 10 MiB would have been ≈1.72 M rows and ≈12 s, inviting proxy and browser timeouts | Accepted; configurable |

## 7. Explicit non-additions

This delta does NOT specify: lemmatization, `Lexeme`, POS population, multiword-expression
