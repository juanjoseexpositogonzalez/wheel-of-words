...
Acceptance: **AC-005-08** — Given every module this capability introduces, when each is inspected
structurally, then it contains no `INSERT`, `UPDATE` or `DELETE` targeting `manual_correction`,
whether expressed through the ORM class or as literal SQL text (§5 AMB-11: dynamically-assembled SQL
text — `%`-format, `str.join`, an interpolated f-string — is a documented, unclosed boundary of static
inspection, not a claim this acceptance makes; §5 AMB-12/AMB-13: the one-hop binding tracker and the
write-verb alias map each cover a bounded, documented set of syntactic forms — an attribute/subscript
binding target, a second assignment hop, and a rebound (not renamed) write-verb name are known,
pinned gaps, not a claim of exhaustive coverage); and given the vocabulary repository with a
...
#### Scenario: No write statement reaches the correction table

- GIVEN every module this capability introduces
- WHEN each is inspected structurally
- THEN none contains an `INSERT`, `UPDATE` or `DELETE` targeting `manual_correction`, expressed
  through the ORM class or as literal SQL text (§5 AMB-11, AMB-12, AMB-13)
...
| **AMB-11** | **AC-005-08 scenario 1 requires structural inspection to find "no `INSERT`, `UPDATE` or `DELETE` targeting `manual_correction`, whether expressed through the ORM class or as SQL text", with no qualifier on how that SQL text comes to exist in source.** Judgment Day round 8 closed the guard's raw-SQL adjacency gaps — whitespace, a newline, a quoted table name, a `main.`-qualified name, `REPLACE INTO`, `TRUNCATE TABLE` (`test_vocabulary_write_guard.py::test_the_raw_sql_adjacency_gaps_named_in_the_brief_are_now_closed`) — and verified, by executing every verb-times-channel combination the acceptance text names, that all six are detected (round 8 evidence, this Judgment Day report). What remains open is a DIFFERENT, narrower channel: a DELETE/INSERT/UPDATE statement whose text is assembled at runtime — `%`-format substitution, `str.join`, or an f-string with an interpolated `{...}` placeholder — evades detection even though it is, at execution time, exactly the SQL text the scenario forbids (`test_dynamic_sql_assembly_is_a_documented_gap`, round 8). **Round 10 correction: the reason above was wrong, verified false by execution.** `ast.walk` on `text("DELETE FROM %s" % "manual_correction")` visits the constant `'manual_correction'` as its own `ast.Constant` — the string DOES occur in a visited constant, contrary to the prior wording. The true mechanism, confirmed for all three forms (round 10 evidence, this Judgment Day report): `_string_literals`'s only literal-combining logic, `_folded_string`, folds an `ast.BinOp` carrying `ast.Add` alone — `%`-format (`ast.Mod`) and `.join()` (a runtime method call, invisible to any `BinOp` fold) are never combined, so `%`-format leaves `'DELETE FROM %s'` and `'manual_correction'` as two separate, non-adjacent constants, and `.join()` leaves `'DELETE'`, `'FROM'` and `'manual_correction'` as three; the verb-plus-table regex needs both inside ONE string, and neither fragment alone carries both. For the interpolated f-string, the gap is different again: `f"DELETE FROM {table_name}"` compiles to an `ast.JoinedStr` whose only `ast.Constant` part is `'DELETE FROM '` — the interpolated `{table_name}` wraps an `ast.Name`, not a `Constant`, so `manual_correction` is not a visited string at all in that specific case. | **Not the same defect as the now-withdrawn original AMB-11.** That entry rested on the premise that an AST pass cannot verify a `Session` receiver; round 6 falsified it — receiver identity was never the blocker, and every ORM-instance idiom is now caught by matching the callee name plus `ManualCorrection` anywhere in the call expression, no receiver inference required. This entry is a different, genuine boundary: closing dynamically-assembled SQL text would require constant propagation or symbolic execution over arbitrary Python to resolve what a variable, a format substitution, or a join ultimately evaluates to at runtime — the same category of unverifiable inference the withdrawn original AMB-11 and three earlier Judgment Day rounds escalated on, this time for string VALUES rather than receiver TYPES. The requirement is not weakened to accommodate it: every `INSERT`/`UPDATE`/`DELETE` expressed as literal SQL text (including every adjacency variant round 8 closed) is caught — confirmed by executing the full six-form matrix AC-005-08's text names. AC-005-08 scenario 1 is amended below to scope "as SQL text" to literal SQL text explicitly, so the acceptance criterion states the boundary the shipped guard actually enforces rather than a broader claim no static scan can make. The ORM-class channel carries its own, separately-registered bounds — see `AMB-12`/`AMB-13`, not restated here. | **Registered per `AGENTS.md` §9. AC-005-08 scenario 1 amended below to name the literal-text scope explicitly, and its reason corrected (round 10)** |
| **AMB-12** | **The one-hop binding tracker (`_one_hop_bindings`) only registers a plain `ast.Name` assignment target, follows exactly one hop, and keeps no lexical-scope association.** Three consequences, each verified by execution (round 10 evidence, this Judgment Day report): (1) an attribute target (`holder.correction = ManualCorrection()`), a `self.x` target, or a subscript target (`d["k"] = ManualCorrection()`) is never registered, so a later `session.add(holder.correction)` referencing that same attribute or key returns `[]` — a false negative; (2) a SECOND hop off an already-registered name — `correction = ManualCorrection(...)` then `alias = correction` then `session.add(alias)` — is never followed, the same false negative one level deeper (`test_a_second_assignment_hop_is_the_documented_uncovered_boundary`); (3) the tracker's `set` of bound names is module-wide, with no per-function scope: `make()` binding `item = ManualCorrection()` and the unrelated `unrelated()` binding a different `item = object()` then `session.add(item)` is FLAGGED, a false positive, because the bare name `item` was bound to `ManualCorrection` somewhere in the module. Tuple/list-unpacking and `for`-loop targets are also never tracked, a pre-existing, already-pinned decision restated here for completeness. | **The under-approximations (1, 2) are accepted, not narrowed — closing either means resolving an arbitrary attribute/subscript chain or a name transitively through the same map, the class of dataflow tracking three earlier Judgment Day rounds already found unreliable for this guard.** The over-approximation (3) is accepted because it is safe by direction: a spurious flag fails a build noisily, it never lets a real write pass silently. Each of the three is pinned by an executable test (`test_a_non_name_assignment_target_is_a_documented_uncovered_boundary`, `test_a_second_assignment_hop_is_the_documented_uncovered_boundary`, `test_the_one_hop_tracker_leaks_across_lexical_scopes_a_known_over_approximation`, `test_tuple_unpacking_targets_are_a_documented_uncovered_boundary`, `test_a_for_loop_target_is_a_documented_uncovered_boundary`), per SPEC-003 §3.6 G3. AC-005-08 scenario 1 is amended below to acknowledge this boundary alongside AMB-11's. | **Registered per `AGENTS.md` §9 (round 10). AC-005-08 scenario 1 amended below** |
| **AMB-13** | **A write verb reached through a REBOUND local name is not resolved.** `_write_verb_aliases` reads only `ast.ImportFrom` nodes, keyed on the imported symbol's original name; a plain `ast.Assign` rebinding an already-resolved name to a further local name — `from sqlalchemy import delete` then `d = delete` then `d(ManualCorrection)` — returns `[]`, verified by execution (round 10 evidence, this Judgment Day report). This is the same class of gap `_write_verb_aliases` closed for a RENAMED import (round 8), one hop deeper for a REBOUND one. | **Accepted, not narrowed, for the same reason as `AMB-12`: resolving a rebinding chain requires the same dataflow tracking already rejected for the `ManualCorrection`-name side.** Pinned by `test_a_rebound_write_verb_is_a_documented_uncovered_boundary`, per SPEC-003 §3.6 G3. AC-005-08 scenario 1 is amended below to acknowledge this boundary. | **Registered per `AGENTS.md` §9 (round 10). AC-005-08 scenario 1 amended below** |
| **AMB-14** | **`_RAW_SQL_PATTERNS`'s whitespace-, quote- and schema-prefix-tolerant regex is not a literal substring match, and its tolerance cuts both ways.** Over-match, verified by execution: prose naming `manual_correction` alongside a write-verb keyword, with no contiguous `verb + table` fragment in the source, can still be flagged — `"""Never update models.manual_correction directly."""` matches because `models.` satisfies the optional schema-prefix group between `update` and `manual_correction`; a literal newline between the verb and the table matches the same whitespace-tolerance that closed the round-8 adjacency gap; `self.manual_correction` in prose matches the same schema-prefix group (`test_prose_matching_the_tolerant_raw_sql_regex_is_flagged_a_known_over_approximation`). Under-match, verified by execution: a table reference with BOTH the schema and the table name independently quoted — `"main"."manual_correction"`, `` `main`.`manual_correction` `` — returns `[]`, because the regex has one quote slot before the schema-prefix group and one between it and the table name, none for a quote closing the schema segment before the dot (`test_a_fully_quoted_schema_and_table_name_is_a_documented_gap`). This under-match is literal SQL text, inside AC-005-08 scenario 1's stated scope; it is also unreachable through this repository's own modules today, because SQLite has no schema support and nothing in this codebase can emit a schema-qualified table reference — that is a reason to register the bound, not to omit it. | **The over-match is accepted, undocumented cost of the round-8 adjacency-tolerance fix: it is safe by direction, the same reasoning as `AMB-12`'s over-approximation.** The under-match (fully-quoted schema-and-table) is accepted and NOT narrowed for this codebase's modules, because it cannot arise while this repository stays SQLite-only (no `ATTACH DATABASE`/cross-schema query is issued anywhere in `wheel_vocabulary`) — narrowing the regex for a form that cannot occur here would add pattern complexity with no reachable benefit, the same "no new detection mechanism" discipline this Judgment Day round closes on. If this repository ever gains schema-qualified access (e.g., a future `ATTACH DATABASE`), this bound must be re-opened. Both directions are pinned by executable tests, per SPEC-003 §3.6 G3. AC-005-08 scenario 1 is NOT amended for the under-match, because the bound is provably unreachable in this codebase today (see GAP column); it IS amended for the general "literal SQL text" scope already carried by `AMB-11`. | **Registered per `AGENTS.md` §9 (round 10). AC-005-08 scenario 1 not further amended for this row — the reachable forms stay inside the existing literal-SQL-text scope** |
