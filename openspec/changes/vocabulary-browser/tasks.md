...
- [x] T13 [TEST] Write `apps/api/tests/unit/test_vocabulary_write_guard.py` — the REQ-005-008 guard that MUST differ from `test_annotation_write_repository_isolation.py`: it permits `select(ManualCorrection...)`/`ManualCorrection.field` reads and forbids only `insert(ManualCorrection)`, `update(ManualCorrection)`, `delete(ManualCorrection)` SQLAlchemy calls and raw `INSERT/UPDATE/DELETE ... manual_correction` SQL text, scanned across every module this capability introduces. Do NOT reuse or extend the existing no-reference guard, and do NOT exempt this capability's modules from it (SPEC-003 §3.4 W1) — this is a distinct, narrower rule (AMB-3). **Correction (Judgment Day round 1, JD-W3-1)**: "scanned across every module this capability introduces" was false as shipped — `_vocabulary_modules()` iterated a fixed `_VOCABULARY_MODULES` manifest and returned only the subset that existed on disk, so `scanned` was by construction a subset of that manifest and the non-vacuity check could never fail on a module the manifest never named. Replaced with a filesystem walk (`_scanned_modules`) over every `"vocabulary"`-named path in the package plus the two shared wiring files (`api/dependencies.py`, `api/main.py`) plus every Alembic migration — see `test_vocabulary_write_guard.py`'s module docstring for the full reasoning, including why an initial whole-package-walk draft was itself corrected after it flagged `book_repository.py`'s legitimate `DeleteImport` cascade delete as a false positive. **Correction (Judgment Day round 2, JD-W3-7)**: "scanned across every module this capability introduces" was STILL an overclaim after round 1 — the token match is a naming CONVENTION, not a manifest of what actually ships (a production module skipping the `"vocabulary"` token in its path is silently excluded, pinned by `test_a_module_outside_the_vocabulary_naming_convention_is_a_known_residual_gap`), the covered-file list wrongly included T12 (a test file outside `_PACKAGE_ROOT`, removed from the list), and the "scanning migrations produces no false positive" claim was disproven and deleted — every migration is scanned with no capability scoping at all, a live false-positive surface pinned by `test_the_scan_reaches_every_migration_regardless_of_capability`. Also added the missing `api/main.py` non-vacuity anchor (Judge A suspect): it sat in `_SHARED_WIRING_FILES` with no entry in `_EXPECTED_PACKAGE_MODULES`, so its silent removal from scope was invisible to §3.3 M2.
- [x] T14 [TEST] Extend T13's module with the two required mutation checks (AC-005-08 scenario 3): a synthetic `insert(ManualCorrection, ...)` and a synthetic `delete(ManualCorrection)`, each in turn, each asserted to produce a violation with the observed failure text recorded in the test docstring. **Correction (Judgment Day round 1, JD-W3-6)**: the insert check's docstring recorded the observed output as `synthetic.py:1 ...`; a verbatim re-run shows `synthetic.py:2` (the call is the source's second line, after the import) — §3.3 M1 requires the recorded output to be verbatim, and this one was transcribed rather than copied. Corrected the recorded line number.
- [x] T15 [TEST] Extend T13's module with the boundary control (AC-005-08 scenario 4/M3): the same forbidden write statement placed in a module OUTSIDE this capability still produces a violation.
- [x] T16 [IMPL] Implement the write-detector in `test_vocabulary_write_guard.py` itself (test-only code, no production module): AST `ast.Call` matching on `insert`/`update`/`delete` imported from `sqlalchemy` with a `ManualCorrection` argument, plus a substring scan over string/`BinOp`-folded literals for `insert into manual_correction`/`update manual_correction`/`delete from manual_correction` (case-insensitive). **Correction (Judgment Day round 1, JD-W3-2)**: "imported from `sqlalchemy`" was aspirational, not implemented — the shipped detector matched any bare `insert`/`update`/`delete` `Name` or any `.insert`/`.update`/`.delete` `Attribute` call by method name alone, with no check on where the name came from. `from sqlalchemy import delete as sa_delete` therefore evaded it entirely (no alias resolution), while `cache.update(ManualCorrection)` false-positived (no origin check), and `session.add(ManualCorrection(...))` — the idiom this codebase's own production code and its own sibling test file actually use — was not recognised as a write idiom at all. Rewrote the detector to resolve import aliases, verify call origin, and cover `session.add`/`add_all` (including via a locally bound variable), `session.delete`, `Query.delete`/`Query.update`, `bulk_insert_mappings`/`bulk_update_mappings`, and `ManualCorrection.__table__` writes, with the residual gaps (opaque row provenance, attribute-mutation-then-commit) documented and each pinned by its own accepted-behaviour test per SPEC-003 §3.6 G3. **Correction (Judgment Day round 2, JD-W3-8/JD-W3-9)**: the module docstring's "never by method name alone" guarantee was itself false — `session.add`/`session.delete`/`Query.delete`/`Query.update`/`bulk_insert_mappings` verify NO receiver origin, so `cache.delete(ManualCorrection)`/`cache.add(ManualCorrection())` are flagged exactly like a real session call, and the violation message asserted `session.` regardless of the actual receiver. Reworded the guarantee to state the over-approximation honestly, changed the message to name the real receiver (`_receiver_name`) and mark these idioms `[receiver origin unverified]`, and pinned the false-positive as accepted (SPEC-003 §3.6 G3) with two new tests. Separately, the binding tracker matched only `ast.Assign` — `correction: ManualCorrection = ManualCorrection(...)` (the idiomatic form in this mypy-strict repository) and `session.add(c := ManualCorrection(...))` both returned `[]`; added `ast.AnnAssign` and `ast.NamedExpr` handling, and documented the tracker's flow-insensitivity (a rebound name stays flagged) and the second-alias gap (`y = x` after `x = ManualCorrection(...)`) as accepted, each with its own test, since the module docstring's prior "Residual gaps" list did not mention either.
