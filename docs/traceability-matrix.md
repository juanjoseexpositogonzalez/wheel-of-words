...
| REQ-002-007 | Ni la clave de agrupación ni la forma mostrada se llaman «lemma» ni «lexeme» | `openspec/changes/text-import/specs/002-text-import/spec.md` — AC-002-10 | `apps/api/tests/unit/test_no_lemma_naming.py` | T1B20 (leg de backend); T1C14, T203 pendientes | En progreso — solo el leg de backend (corte 1b) está cubierto; los legs de frontend (corte 1c) y de columnas persistidas (corte 2) siguen pendientes (spec §1.2, requisito transversal) |
