...
- [x] T13 [TEST] Write `apps/api/tests/unit/test_vocabulary_write_guard.py` — the REQ-005-008 guard that MUST differ from `test_annotation_write_repository_isolation.py`: it permits `select(ManualCorrection...)`/`ManualCorrection.field` reads and forbids only `insert(ManualCorrection)`, `update(ManualCorrection)`, `delete(ManualCorrection)` SQLAlchemy calls and raw `INSERT/UPDATE/DELETE ... manual_correction` SQL text, scanned across every module this capability introduces. Do NOT reuse or extend the existing no-reference guard, and do NOT exempt this capability's modules from it (SPEC-003 §3.4 W1) — this is a distinct, narrower rule (AMB-3). **Rewritten after Judgment Day terminated ESCALATED for a third round**: rounds 1-2 narrowed scope to a `"vocabulary"`-token naming convention that a future production module skipping the convention could silently evade, with no way to notice. The file was rewritten from scratch: `_scanned_modules` now walks the ENTIRE `wheel_vocabulary` package unconditionally (`rglob("*.py")`, no filter) plus every Alembic migration, which makes "every module this capability introduces" true by construction — the scanned set is a strict superset, not a convention. `book_repository.py` (`002-text-import`'s `DeleteImport` cascade delete) is the one named exemption, applied at the violation-aggregation step (`_write_violations`) rather than by excluding it from the walk; the walk still reaches it (`test_the_only_exemption_is_book_repository_and_it_is_scanned`). Two tests prove the exemption is load-bearing rather than decorative: `test_the_exempted_write_is_real_not_inert` (the suppressed write is genuine, verified by calling the detector directly on the real file) and `test_removing_the_exemption_fails_the_scan` (dropping the entry fails the guard against the real tree, RED verified by execution before the fix and GREEN after). `test_no_exempt_module_carries_the_vocabulary_naming_token` makes it impossible for this capability's own modules to ever be exempted. Migrations keep no exemption mechanism at all; `test_a_migration_write_is_flagged_with_no_exemption_mechanism` replaces the round-2 test that a judge proved would pass even with the detector stubbed to return `[]` — the rewritten version calls `_write_violations` end to end against a synthetic migration.
- [x] T14 [TEST] Extend T13's module with the two required mutation checks (AC-005-08 scenario 3): a synthetic `insert(ManualCorrection, ...)` and a synthetic `delete(ManualCorrection)`, each in turn, each asserted to produce a violation with the observed failure text recorded in the test docstring. **Carried into the rewrite**: `test_a_synthetic_insert_would_be_caught` and `test_a_synthetic_delete_would_be_caught` keep the same shape; both verbatim outputs were re-run and re-recorded during the rewrite (`synthetic.py:2 ORM write call insert(ManualCorrection)` / `delete(ManualCorrection)`), matching the prior recorded values exactly. Extended with `test_a_write_placed_in_the_vocabulary_repository_would_be_caught`, which appends the same two mutations to the REAL `vocabulary_repository.py` source rather than a synthetic snippet, matching AC-005-08 scenario 3's literal wording.
- [x] T15 [TEST] Extend T13's module with the boundary control (AC-005-08 scenario 4/M3): the same forbidden write statement placed in a module OUTSIDE this capability still produces a violation. **Rewritten as two tests**: `test_the_exemption_boundary_holds` is the pure-detector control (a synthetic module carrying no exemption still violates); `test_removing_the_exemption_fails_the_scan` is the boundary control made meaningful by a REAL exemption — dropping `book_repository.py` from `_EXEMPT_WRITE_MODULES` and re-running `_write_violations` against the real tree fails with that module's actual cascade delete. Rounds 1-2's version of this control was inert: no exemption existed yet for it to test the removal of.
- [x] T16 [IMPL] Implement the write-detector in `test_vocabulary_write_guard.py` itself (test-only code, no production module): AST `ast.Call` matching on `insert`/`update`/`delete` imported from `sqlalchemy` with a `ManualCorrection` argument, plus a substring scan over string/`BinOp`-folded literals for `insert into manual_correction`/`update manual_correction`/`delete from manual_correction` (case-insensitive). Rounds 1-2 corrected the detector itself (import-alias resolution, origin checks, `session.add`/`add_all`/`delete` binding tracking including `AnnAssign`/`NamedExpr`, `Query.delete`/`.update`, `bulk_insert_mappings`/`bulk_update_mappings`, `ManualCorrection.__table__` writes) — none of that detection logic changed in the round-3 rewrite; every AST-matching function carried forward unmodified and re-verified green. **Rework (round 3, this work unit)**: what changed is the scan/exemption architecture around the unchanged detector (see T13) and claim discipline in the module docstring — the "Known gaps" section is now explicitly non-exhaustive ("known gaps include", never "this list is exhaustive"), and six new gaps are pinned by tests that call the detector against the uncovered case rather than merely asserting a label: a `sqlalchemy.sql`/`sqlalchemy.sql.expression` submodule import, `session.merge(...)`, `add_all` with a tuple or generator, tuple-unpacking and `for`-target bindings, and nine raw-SQL adjacency variants (`REPLACE INTO`, `TRUNCATE`, whitespace variants, quoted/schema-qualified names, `str.join`/`%`-format/f-string assembly).
