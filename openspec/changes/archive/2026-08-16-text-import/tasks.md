...
| AC-19 | REQ-002-014 | 1c | T1C07, T1C09 | `apps/web/tests/components/FrequencyTable.test.tsx` :: "renders each received display form, grouping key, and frequency in server order", :: "explains the shown text and grouping key without linguistic claims"; `apps/web/tests/contracts/no-linguistic-rules.test.ts::test_import_modules_have_no_linguistic_rules` |
...
12. **Gap found in cut 1c's frontend linguistic-rule guard and CLOSED by a remediation work unit.**
    `apps/web/tests/contracts/no-linguistic-rules.test.ts` originally used a raw regex over manifest
    source text (`.sort(`, `toLowerCase(`, `localeCompare(`, `normalize(`, and normalization-form
    literals). That left two problems: comments explaining the prohibition could false-positive, and
    several real client-side transformations were invisible (`toSorted`, `reverse`, and
    `Intl.Collator`). **Resolution adopted:** the guard now uses the TypeScript compiler API already
    present in `apps/web` to parse each manifest source file with `ts.createSourceFile` and walk the
    AST. It reports forbidden method calls (`sort`, `toSorted`, `reverse`, `localeCompare`,
    `toLowerCase`, `normalize`), `new Intl.Collator(...)`, `Intl.Collator(...)`, and normalization-form string literals
    (`NFC`, `NFD`, `NFKC`, `NFKD`) with file, line, kind, and matched text. Inline synthetic tests pin
    both directions permanently: comments naming the forbidden transformations produce zero
    violations, while sort/toSorted/reverse/localeCompare/new Intl.Collator/Intl.Collator/toLowerCase/normalize/NFC
    each produce exact structured violations. The shipped-source assertion still runs over the same
