...
| Field | Value |
|-------|-------|
| Estimated changed lines (slice 1, this document's scope) | **≥2,335, a lower bound** — the per-unit table below sums to exactly 2,335 across the 10 estimated units; WU2b is explicitly unestimated (§Snapshot isolation carved out of WU2) and adds an unknown amount on top, so 2,335 is a floor, not a range maximum. **Correction (Judgment Day round 8):** a prior revision of this row read "~2,270-2,335 (range from two independent groupings below)" — no second grouping exists anywhere in this document, and 2,270 does not appear in the per-unit table or any subtotal; it was an unsupported number that happened to produce round-looking ratios below |
| Proposal's own estimate | ~400-430 |
| Design's own estimate | ~1,080 |
| This estimate vs. proposal | **~5.4x-5.8x** (2,335 / 430 to 2,335 / 400; a range because the proposal itself is a range) |
| This estimate vs. design | **~2.2x** (2,335 / 1,080) |
| 400-line budget risk | **High** |
| Chained PRs recommended | **Yes** |
| Suggested split | 11 work units, see below. **Corrected (Judgment Day round 12, tasks.md finding):** a prior revision of this row claimed no single unit stays observable AND under 400 — false, WU9 is exactly that (`Yes`, ~235 lines, see the unit table below and §Honest answer on the 400-line floor). WU2b was carved out of WU2 after apply (see §Snapshot isolation carved out of WU2). WU3 was briefly shrunk and split into WU3a/WU3b/WU3c after Judgment Day round 4 on the premise that an AST pass cannot verify a `Session` receiver; round 6 falsified that premise and the split was withdrawn — see Phase 3's split-and-withdrawal note |
...
**The two independently user-observable units land on opposite sides of 400 lines.** WU6 is the
first HTTP-observable unit (endpoint reachable, verifiable by `curl`/OpenAPI) and lands at
~400 — right at the edge, not comfortably under it. WU9 is the first UI-observable unit and is a
comfortable ~235 — the one independently user-observable unit that DOES stay under 400.
**Correction (Judgment Day round 10, tasks.md finding): a prior revision of this paragraph
claimed no user-observable unit stays under 400 lines, then stated WU9's ~235 in the very next
sentence** — WU9 is marked `Yes` (full user-visible slice) in the unit table above, and ~235 is
under 400 by the same table; the two sentences contradicted each other. Every OTHER unit strictly
under 400 (WU1, WU2, WU3, WU4, WU5, WU7, WU8, WU10) is backend-internal or non-shipped-surface
work with no observable output by itself — this list previously omitted WU3 (~390), also strictly
under 400 and also backend-internal (Judge A, round 10). The floor for *a single fully-observable
slice* (repository → route → frontend, WU2+WU5+WU6+WU9) is ~1,190 lines — well above 400. Eleven
work units is the finest useful split found; splitting WU6 or WU3 further (e.g. route+schema
separate from API tests) is possible but adds PRs without changing the fundamental floor.
...
branch whose claims could not be proven. The split's premise, registered as the ORIGINAL spec §5
AMB-11 (withdrawn — see below), was that an AST pass over untyped Python cannot verify a receiver
is a `Session`, so ORM-instance forms needed a second mechanism — Phase 3b's runtime
`before_cursor_execute` listener — to be covered at all, with Phase 3c closing **three** remaining
detector gaps (T21d: submodule-import resolution; T21e: raw-SQL adjacency; T21f: `__table__`
writes) — not two, as an earlier revision of this note claimed. **Judgment Day round 6 falsified
the receiver-inference premise in one paragraph**: `session.add(ManualCorrection(...))`,
...
receiver is. Detection was rewritten on exactly that rule (T13/T16 below), which directly closed
T21b's `session.add(ManualCorrection(...))` idiom and T21f's `__table__` form, and made T21d's
submodule-import concern moot — the rewrite never resolves import origin at all, for either the
write verb or the class name, so a submodule-imported write is caught identically to a top-level
one. **Correction (Judgment Day round 8, tasks.md finding): round 6's claim that the rewrite
"closed every form the split existed to cover" was premature by one gap.** T21e's raw-SQL
adjacency gap — whitespace, a newline, a quoted table name, a `main.`-qualified table name,
`REPLACE INTO`, `TRUNCATE TABLE` — was NOT closed by the round-6 rewrite; it stayed open,
documented as a known, non-exhaustive gap
(`test_raw_sql_adjacency_gaps_are_not_exhaustive`, verified by execution to return `[]` against all
six variants), until Judgment Day round 8 closed it (T13/T16's round-8 correction below,
`test_the_raw_sql_adjacency_gaps_named_in_the_brief_are_now_closed`). Phase 3b and Phase 3c are
withdrawn, their T21a-T21g tasks deleted. Spec §5's ORIGINAL AMB-11 (the receiver-inference
premise) was withdrawn and `spec.md` reverted to its pre-AMB-11 text at that time (AC-005-08 and
its scenario 1 read exactly as they did before this split, byte-for-byte against `b874f62`). Round
8 registers a NEW, unrelated AMB-11 in the same table slot — a different finding entirely
(dynamically-assembled SQL text is not resolved by static inspection, not a claim about receiver
inference) — see `spec.md` §5.
...
- [x] T35 [TEST] Write `apps/api/tests/api/test_vocabulary_route.py` (mirrors `test_annotation_route.py`): `GET /api/v1/imports/{id}/vocabulary` returns 200 with the design's shape for a seeded import; unknown id → 404 `IMPORT_NOT_FOUND` (AC-005-05); an error body contains no textual form, lemma, stack trace, or path (REQ-003-019 inherited); `annotation.v1.json` byte-identical before/after (AC-005-04); served OpenAPI lists the new path alongside the two unchanged annotation operations; **the parsed `groups` list equals a literal expected ordering** — seed a fixture whose correct design-D5 order (`occurrence_count DESC, lemma, pos`, `NULL` before any string in both halves) is known in advance and assert the parsed list equals that literal, positionally; **and** two identical `GET` requests with no intervening write return equal response bodies including `groups` order (AC-005-01 scenario 3). The repeat-request check proves serialization stability; the literal-order check proves the order is the specified one, which a stable-but-wrong order would otherwise pass. Compare positionally, never as a set or a sorted copy. Seed tied `occurrence_count` groups and at least one `NULL` key half so the tie-break and `NULL`'s position are proved at the wire level and not only inside the repository.
- [x] T36 [IMPL] Create `apps/api/src/wheel_vocabulary/api/schemas/vocabulary.v1.json` (Draft 2020-12, mirrors `annotation.v1.json`'s shape): `id`, `group_count`, `total_occurrence_count`, `groups[]` with `lemma`/`pos`/`occurrence_count`.
- [x] T37 [TEST] Extend `test_no_lemma_naming.py`: add `"vocabulary.v1.json"` to `_EXPECTED_SCHEMA_FILES` (`:406`); add a `_SCHEMA_OWNERS["vocabulary.v1.json"]` entry (`:219-229`) with `OwningDefinition` scoped to the group shape, `exempt={"lemma"}`; extend `_OPENAPI_OWNERS` (`:230-236`) with a second `OwningDefinition` for the served `VocabularyGroupResponse` component.
- [x] T38 [IMPL] Create `apps/api/src/wheel_vocabulary/api/routes/vocabulary.py`: `GET /api/v1/imports/{import_id}/vocabulary`, thin adapter over `ReadVocabulary`, mirrors `read_import`/`read_annotation`'s shape (`X-Schema-Version` header, `ImportNotFoundError` on `None`).
- [x] T39 [TEST] Extend `test_no_lemma_naming.py::_LEMMA_OWNING_FILES` with `"api/routes/vocabulary.py": frozenset({"lemma"})`.
- [x] T40 [IMPL] Register the router in `apps/api/src/wheel_vocabulary/api/main.py` (`app.include_router(vocabulary_router_module.router)`, alongside the existing three).
- [x] T41 [TEST] Run T35 green; re-run the full `003-lemmatization-pos` acceptance suite unweakened (AC-005-04 scenario 2); run the runtime harness once manually.
...
- [x] T55 [TEST] Write `apps/web/tests/components/VocabularyBrowser.test.tsx` (mirrors `AnnotationTable.test.tsx`): a mocked response renders lemma/pos/count verbatim (AC-005-10 scenario 2); a `(null, null)` group, a `(lemma, null)` group, and a tagged group each carry a distinct text label (AC-005-03 scenario 4); an unmapped POS tag degrades to the raw tag, no blank cell (AC-005-10 scenario 3); zero interactive controls submit a correction (AC-005-08 scenario 5).
...
- [x] T57 [TEST] Register the component in both frontend guard manifests now that T56 has created it: add `"src/components/VocabularyBrowser.tsx": new Set(["lemma"])` to `no-lemma-naming.test.ts::LEMMA_OWNING_FILES` (`:60-68`) and add the same path to `no-linguistic-rules.test.ts::FRONTEND_FEATURE_MODULES` (`:28-38`). Then run both guards: zero matches for grouping/lemmatization/tagging/normalization/precedence identifiers (AC-005-10 scenario 1). Registering before T56 would point a checked manifest at a file that does not exist — that is why these two entries are here and not in Phase 8.
- [x] T58 [IMPL] Modify `apps/web/src/pages/ImportPage.tsx`: add a "Ver vocabulario" trigger and state slice (mirrors the existing `handleAnnotate`/`AnnotateState` pattern, `:14-50`), calling `getVocabulary(result.id)` and rendering `<VocabularyBrowser>` on success.
- [x] T59 [TEST] Write `apps/web/e2e/vocabulary.spec.ts` (mirrors `annotation.spec.ts`, 25 lines): import → annotate → view vocabulary → grouped table with counts is visible.
- [x] T60 [TEST] Run T55/T57 green; run the E2E harness; run `pnpm run typecheck`, `pnpm run lint`, `pnpm run test:coverage`.
...
write — the then-shipped guard did not detect (spec §5 AMB-11, original). Round 6 closed the
ORM-instance and `__table__` forms inside T12-T17 itself, by rewriting the detector on a rule that
never inspects the receiver (see Phase 3's split-and-withdrawal note and T13's round-6 correction).

**Correction (Judgment Day round 8, tasks.md finding).** Round 6's claim was one gap short: Phase
3c carried THREE tasks, not two (T21d submodule-import resolution, T21e raw-SQL adjacency, T21f
`__table__` writes), and T21e's raw-SQL adjacency gap (whitespace, quoting, schema-qualification,
`REPLACE INTO`, `TRUNCATE TABLE`) was NOT closed by the round-6 rewrite — it stayed open,
documented and pinned as a known, non-exhaustive gap, until round 8 closed it inside T12-T17 again
(T13/T16's round-8 correction). Round 8 also found and closed two further gaps outside the
original Phase 3b/3c split's scope entirely — a renamed write-verb import
(`from sqlalchemy import delete as sa_delete`) and a `ManualCorrection` instance constructed on one
line and passed by name on a later line (a one-hop binding tracker) — and registered a NEW spec §5
AMB-11 (a different finding: dynamically-assembled SQL text, `%`-format/`str.join`/an interpolated
f-string, is not resolved by static inspection and stays open), amending AC-005-08 scenario 1's
wording to scope "as SQL text" to literal SQL text. Measured against that amended text,
**REQ-005-008 is fully discharged by T12-T17**: every ORM-class and literal-SQL-text form
AC-005-08 names is detected, verified by executing the full verb-by-channel matrix (round 8
Judgment Day report). There is no `T21a-T21g` for this map to cite; all detector work through
round 8 lives inside T13/T16.

**Correction (Judgment Day round 10, tasks.md finding).** "Fully discharged" above was measured
only against the SQL-text channel `AMB-11` carves out; the ORM-class channel carries its own
bounds, found and registered this round as spec §5 `AMB-12` (the one-hop binding tracker's
single-`Name`, single-hop, module-wide-set limits) and `AMB-13` (a write verb reached through a
rebound, not renamed, local name). Both are accepted, pinned gaps — narrowing either re-adds the
dataflow tracking three earlier Judgment Day rounds already rejected for this guard — and
AC-005-08 scenario 1 is amended a second time to cite them alongside `AMB-11`. Round 10 also found
`_RAW_SQL_PATTERNS`'s quote-tolerant regex cuts both ways (`AMB-14`): it over-matches prose
carrying a write-verb keyword with no contiguous verb-plus-table fragment, and under-matches a
table reference with the schema and the table independently quoted — the latter is literal SQL
text inside AC-005-08's stated scope but is provably unreachable while this repository stays
SQLite-only, so scenario 1 is NOT amended for it. **REQ-005-008 stays fully discharged**, now
measured against AC-005-08 as amended for `AMB-11`, `AMB-12` and `AMB-13` together: every form the
amended text names is detected, and every bound it excludes is registered and pinned by an
executable test in `test_vocabulary_write_guard.py` (round 10 Judgment Day report).

**Correction (Judgment Day round 12, tasks.md finding).** The paragraph above's claim that the
schema-and-table-independently-quoted form "is provably unreachable while this repository stays
SQLite-only" is false, verified by execution: `main`/`temp` are built-in SQLite schemas needing no
`ATTACH DATABASE`, and both `DELETE FROM "main"."manual_correction"` and the backtick-quoted form
execute successfully against plain `sqlite3` (round 12 evidence, this Judgment Day report);
`database_url` (`infrastructure/settings.py`) is an env-overridable pydantic setting and
`create_engine_from_url` (`infrastructure/persistence/engine.py`) imposes no dialect restriction, so
"SQLite-only" describes today's deployment default, not a structural impossibility. The same test
module already contradicted this premise: `test_the_raw_sql_adjacency_gaps_named_in_the_brief_are_
now_closed` parametrizes `text("DELETE FROM main.manual_correction")` under the id
`"schema-qualified"` and asserts it IS caught. This round also reshapes AC-005-08 scenario 1 itself,
closing the deeper problem the false premise was a symptom of: eleven rounds enumerating the
detector's bounds as an exception list kept finding a bound the list did not carry (round 11 found a
fifth over-approximation class the four `AMB-11`-`AMB-14` rows did not name, plus this false
premise). Spec §5 `AMB-11` through `AMB-14` are collapsed into one row, `AMB-15`; scenario 1 now
states that the detector's scan reaches every module in scope, reports no unexempted violation, has
every mechanism and bound defined and kept true by a named test in its own test module, and that the
exempt set cannot grow without its boundary-holding test — with the module itself, not this note or
the scenario text, as the authoritative record of coverage and of bounds. **REQ-005-008 stays fully
discharged**, now measured against the reshaped scenario 1: the scan reaches every anchor
(`test_the_scan_reaches_every_expected_module`, `test_the_scan_fails_closed_when_an_anchor_is_
missing`, its parametrize list hardcoded per round 12 below), every write form the detector is built
to catch is caught with the one documented exemption held to exact-set equality
(`test_no_module_writes_to_manual_correction`, `test_the_exempt_set_is_exactly_book_repository`),
and every mechanism and every regex tolerance component is independently mutation-pinned
(round 12 Judgment Day report — eighteen previously-combination-only tolerance components, the
`_folded_string` fold arm, and the `_names_manual_correction` attribute arm).
