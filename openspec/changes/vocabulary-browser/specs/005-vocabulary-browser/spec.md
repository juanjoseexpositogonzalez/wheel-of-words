...
boundary an exact-set-equality test enforces; and every mechanism the detector implements and every
bound it accepts is defined, and kept true, by a named test in its own test module, which is the
...
- AND every detection mechanism the detector implements and every bound it accepts is defined, and
  kept true, by a named test in the detector's own test module (§5 AMB-15) — a mechanism or a bound
  whose enforcing test is missing or deleted is a failure of this scenario, not merely an
  undocumented gap; the module's bounds are its authoritative, non-exhaustive record — a bound not
  yet found is a bound not yet found, not a bound this scenario claims does not exist
