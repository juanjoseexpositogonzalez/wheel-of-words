...
| REQ-003-001 | El runtime del backend está fijado a Python 3.12; el pin es prerrequisito duro antes de cualquier dependencia NLP | `openspec/changes/lemmatization-pos/specs/003-lemmatization-pos/spec.md` — AC-003-01 | `apps/api/tests/unit/test_python_pin.py` | 1.1–1.3 (Phase 1, `lemmatization-pos`) | Cumplido — corte 1 de `lemmatization-pos` (capability `003-lemmatization-pos`): `pyproject.toml` declara `requires-python = ">=3.12,<3.13"`, `.python-version` fijado a `3.12` vía `uv python pin 3.12`, `[tool.mypy] python_version = "3.12"` coincide con el runtime resuelto (`3.12.13`). `spacy` y `en-core-web-sm` (dependencia URL-pinned, no `spacy download`) se instalan desde wheels precompiladas — cero compilación de C/Cython confirmada en el log de `uv add` (OQ-3, AC-003-01 escenario 3) |
