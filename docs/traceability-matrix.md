...
| REQ-002-007 | Ni la clave de agrupación ni la forma mostrada se llaman «lemma» ni «lexeme» | `openspec/changes/text-import/specs/002-text-import/spec.md` — AC-002-10 | `apps/api/tests/unit/test_no_lemma_naming.py` | T1B20 (leg de backend); T1C14 (leg de frontend); T203 pendiente | En progreso — los legs de backend (corte 1b) y de frontend (corte 1c) están cubiertos; el leg de columnas persistidas sigue pendiente y se completa en el corte 2 (spec §1.2, requisito transversal). El leg de backend es estructural (AST + claves JSON + documento OpenAPI servido); el leg de frontend es una búsqueda de texto plano sobre `apps/web/src/` sin exención de comentarios, más estricta que el leg de backend, no menos — ver el docstring del módulo de test para la desviación documentada frente al criterio AST que menciona AC-002-10 |
