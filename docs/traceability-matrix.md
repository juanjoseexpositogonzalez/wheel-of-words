...
| REQ-003-002 | El objeto de valor de anotación es puro y vive en el dominio; el guard de aislamiento del dominio se extiende a las bibliotecas NLP | `openspec/changes/lemmatization-pos/specs/003-lemmatization-pos/spec.md` — AC-003-02 | `apps/api/tests/unit/test_domain_isolation.py` | 1.4–1.5 (Phase 1, `lemmatization-pos`) | En progreso — corte 1: solo se cierra la extensión del guard AST de aislamiento (`_FORBIDDEN_IMPORT_PATTERN` ahora incluye `thinc`/`stanza`, además de `fastapi`/`sqlalchemy`/`pydantic`/`spacy`), verificada con un mutation-check real sobre `domain/frequency.py` (revertido). El value object `LinguisticAnnotation` y el resto del contrato de REQ-003-002 llegan en el corte 2 (`feat/spec-003-02-domain-port`) |
